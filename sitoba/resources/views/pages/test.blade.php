...
<script>
    ...
    function processPayment() {
        ...
        const totalAkhir = subtotal - ((diskon / 100) * subtotal); // Hitung total akhir dengan diskon
        const kembalian = bayar - totalAkhir;

        if (kembalian >= 0) {
            // Kirim data pembayaran ke server menggunakan AJAX
            fetch("{{ route('processPayment') }}", {
                ...
                .then(data => {
                    alert(data.message);
                    // Arahkan kembali ke halaman indeks setelah pembayaran berhasil
    ...
    function updateKembalian() {
        ...
        // Format kembalian ke dalam format mata uang Rupiah (Rp) tanpa digit desimal
        const formattedKembalian = new Intl.NumberFormat('id-ID', {
            style: 'currency',
            currency: 'IDR',
            minimumFractionDigits: 0 // Tampilkan tanpa digit desimal
        }).format(kembalian);
